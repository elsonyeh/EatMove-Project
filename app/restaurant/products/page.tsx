...
export default function RestaurantProductsPage() {
    ...
    try {
      ...
      if (data.success) {
        const updatedProducts = products.map((product) =>
          product.dishid === currentProduct.dishid
            ? {
                ...product,
                name: formData.name,
                description: formData.description,
        ...
        toast({
          title: "✅ 商品已更新",
          description: "商品資訊已成功寫入資料庫",
        });
      } else {
        toast({
          title: "❌ 更新失敗",
          description: data.message || "無法更新商品",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleDeleteProduct = async (dishid: string) => {
    try {
      const res = await fetch("/api/products", {
        method: "DELETE",
        headers: {
      ...
      if (data.success) {
        // 從前端狀態移除該商品
        setProducts(products.filter((product) => product.dishid !== dishid));

        toast({
          title: "✅ 商品已刪除",
          description: "商品已從資料庫中移除",
        });
      } else {
        toast({
          title: "❌ 刪除失敗",
          description: data.message || "無法刪除商品",
          variant: "destructive",
        });
      }
    } catch (error) {
              ...
              {products.map((product) => (
                <Card key={product.id} className="overflow-hidden border-none shadow-md">
                  ...
                  <CardFooter className="flex justify-between">
                    ...
                    <Button variant="destructive" size="sm" onClick={() => handleDeleteProduct(product.dishid)}>
